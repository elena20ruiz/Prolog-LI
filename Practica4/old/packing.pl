...
writeClauses:-
	...
	fill,
	eachXYhasAtMostOneB,
	eachBhastAtMostOneStart,
    	true.

%Cualquier valor dentro del bloque
posRec(B,X0,Y0,X1,Y1):-
			rect(B,W,H),
			between(1,W,I),
			between(1,H,J),
			X1 is I - 1 + X0,
...
displaySol(M):- height(H), nl, yCoord(Y), Y2 is H-Y+1, nl, xCoord(X), rect(B), member(fills-B-X-Y2, M),space(B),write(B) ,write(' '), fail.
displaySol(_):- nl,nl.

space(B) :- member(B, [1,2,3,4,5,6,7,8,9]), write(' '), !.
space(_).


%Sea X,Y cualquier posicion de la tabla
% Para toda combinación de fills-_-X-Y talque
% B existes, como mucho una puede ser cierta
...
% Sea B todo rect existente
% para toda combinación X-Y de la tabla
% max solo puede comenzar en una
eachBhastAtMostOneStart:-
			rect(B),
...
%Dado un  bloque B de dim WxH
%Sean coordenadas X0-Y0 c. iniciales
%Calcula X1-Y1 como coord finales.
%Si X1-Y1 està fuera de la tabla, entonces no puede comenzar en X0-Y0.
% Sol:  si no dentro no start x0-y0 => dentro o no start
putStart :- 
	    rect(B),
	    insideTable(X0,Y0),
	    posRec(B,X0,Y0,X1,Y1),
	    \+insideTable(X1,Y1),
	    writeClause([\+starts-B-X0-Y0]),
	    fail.
putStart.

%Completar el cuadrado una vez que se fija el start.
%Caso 1: Start-B-X-Y = TRUE
fill :-
	rect(B),
	insideTable(X0,Y0),
	%para cualquier coord del bloque
	posRec(B,X0,Y0,X,Y),
	insideTable(X,Y),
	writeClause([\+starts-B-X0-Y0, fills-B-X-Y]),
	fail.

%Caso 1: Start-B-X-Y = FALSE
%Si comienza en B-X-Y no esta X0-Y0
fill :-
	rect(B),
	insideTable(X0,Y0),
	%para cualquier coord fuera del bloque
	\+posRec(B,X0,Y0,X,Y),
	insideTable(X,Y),
	writeClause([\+starts-B-X0-Y0, \+fills-B-X-Y]),
	fail.

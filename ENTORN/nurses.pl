...
%% Queremos programar las enfermeras que trabajan en un hospital.
%% Por cada hora, sabemos el número mínimo (cada día el mismo) de enfermeras necesarias.
%% Cada enfermera todos los días tiene el mismo horario: comienza a trabajar XX: 00, luego trabaja para
%% tres horas consecutivas, descanse por una o dos horas, y luego trabaje tres
%% horas nuevamente, por ejemplo: trabajo 23:z<00-02:00, descanso 2:00-3:00, trabajo 3:00-6:00.
%% Además, cada enfermera tiene un rango de horas (horas) en que no puede trabajar.
%% Encuentra horas de trabajo para cada enfermera, dadas todas las restricciones.
...
eachHatLeastMinK:-
  needed(H,K), hour(H), findall(worksNH-N-H,nurse(N),Lits), atLeast(K,Lits), fail.
eachHatLeastMinK.

...
%   2. canwork-G-T-H mean: "gangster puede trabajar en la tarea T"

writeClauses(K):-
    initClauseGeneration,
    eachHourEachGangsterAtMostOneTask,
    eachGangsterNotConsecutiveDiferentTask,
    propagate,
    completeTask,
    atMostKConsecutive(K),
    true,!.
...
%Sea G un gangster y esta libre en la hora H.
% Si la hora anterior estaba bloqueado
% Puede hacer una tarea nueva
eachGangsterNotConsecutiveDiferentTask:- available(G,H),
                                          H1 is H - 1,
                                          H1 > -1,
                                          blocked(G,H1),
                                          needed(T,H,_),
                                          writeClause([canwork-G-T-H]),
                                          fail.


%Sea G un gangster y esta libre en la hora H.
% Si la hora anterior no estaba bloqueado.
% Sea  T para H y T para H1 (necesario) Si T \= T1,
% Si ha hace T no podra hacer T1
eachGangsterNotConsecutiveDiferentTask:-    available(G,H),
                                            H1 is H - 1,
                                            H1 > -1,
                                            \+ blocked(G,H1),
                                            ...
                                            T \= T1,
                                            needed(T1,H1,_),
                                            writeClause( [\+does-G-T1-H1, \+canwork-G-T-H]),
                                            fail.
eachGangsterNotConsecutiveDiferentTask.


%Sea la Tarea T, que se ha de hacer en H, por N gangsters
% Para cada gangsters disponible, exactamente N pueden trabajar,
% si ya trabajan N, entonces el no trabaja.
completeTask :- needed(T,H,N),
                findall(does-G-T-H, available(G,H),Lits),
                atLeast(N,Lits),
                fail.
completeTask.

%Si no puede trabajar en la tarea T, no la hace
propagate :-
             available(G,H),
             needed(T,H,_),
             writeClause([canwork-G-T-H, \+does-G-T-H]),
             fail.
propagate.

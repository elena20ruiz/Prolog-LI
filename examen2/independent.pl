...
% VARIABLES
% cityuse-C-S :: City P useStation S
...
writeClauses(K):-
    ...
    someCitiesByTwoStations,
    ifUsedScityUseS,

    someCitiesByTwoStations,   %%%% NEW1
...
ifUsedScityUseS:-
	station(S),
	city(C,List),
        member(S,List), % City C has station S
	writeClause([used-S,\+cityuse-C-S]),
	fail.
ifUsedScityUseS.
...
importants([1,5,10,20]).

important(C) :- importants(L), member(C,L).


eachCityAtLeastOneStation:-
    city(C,_),
    \+important(C),
    findall(cityuse-C-S, station(S), Lits),
    atLeast(1, Lits),
    fail.     
eachCityAtLeastOneStation.
...
cost(M,Cost):-
    findall(used-A, member(used-A,M), Stations),
